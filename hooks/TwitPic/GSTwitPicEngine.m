...
#import "http/ASIFormDataRequest.h"
#import "http/ASINetworkQueue.h"

#import "oauthconsumer/OAConsumer.h"
#import "oauthconsumer/OARequestHeader.h"
...
- (void)dealloc {
  _delegate = nil;
  [_queue release];
  [TWITTER_OAUTH_CONSUMER_KEY release];
  [TWITTER_OAUTH_CONSUMER_SECRET release];
  [super dealloc];
}
...
- (void)uploadPicture:(UIImage *)picture withMessage:(NSString *)message {
  if ([TWITPIC_API_VERSION isEqualToString:@"1"]) {
    ...
    [request addData:UIImageJPEGRepresentation(picture, 1.0) forKey:@"media"];
...
- (void)setConsumerKey:(NSString *)key {
	TWITTER_OAUTH_CONSUMER_KEY = [key retain];
}

- (void)setConsumerSecret:(NSString *)secret {
	TWITTER_OAUTH_CONSUMER_SECRET = [secret retain];
}
...
#elif TWITPIC_USE_TOUCHJSON
        NSError *error = nil;
        response = [[CJSONDeserializer deserializer] deserialize:[request responseData] error:&error];
        if (error != nil) {
