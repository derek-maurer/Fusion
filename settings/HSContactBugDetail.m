...
@implementation HSContactBugDetail
@synthesize addingNew, bug;
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    if (!addingNew)
        shortDes.text = [[bug objectForKey:@"ShortDescription"] stringByReplacingOccurrencesOfString:@"+" withString:@" "];
    ...
    if (!addingNew)
        longDes.text = [[bug objectForKey:@"LongDescription"] stringByReplacingOccurrencesOfString:@"+" withString:@" "];
    if (!addingNew)
        longDes.editable = NO;
    ...
    if (addingNew)
        ...
        [button setTitle:@"I suffer from this too!" forState:UIControlStateNormal];
...
- (void)buttonPressed:(id)sender {
    if (addingNew) {
        if (shortDes.text.length > 0 && longDes.text.length > 0) {
            self.bug = [NSMutableDictionary dictionary];
            [bug setObject:shortDes.text forKey:@"ShortDescription"];
            [bug setObject:longDes.text forKey:@"LongDescription"];
            NSString *ID = [NSString stringWithFormat:@"%@%@%@",[[UIDevice currentDevice] uniqueIdentifier],shortDes.text,[[NSDate date] description]];
            [bug setObject:[self SHA1:ID] forKey:@"ID"];
            [bug setObject:[[UIDevice currentDevice] uniqueIdentifier] forKey:@"UDIDOfReporter"];
            [bug setObject:[NSArray arrayWithObject:[[UIDevice currentDevice] uniqueIdentifier]] forKey:@"Reporters"];
            ...
            [prefs setObject:bug forKey:@"NewBug"];
            ...
            [center sendMessageName:@"ReportNewBug" userInfo:bug];
            
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Bug" message:@"I'm sorry you are having this issue. The developer has been notified and he will start working on it as soon as he can!" delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil];
            [alert show];
            [alert release];
            ...
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Error" message:@"Please enter a short and long description of the bug" delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil];
            [alert show];
            [alert release];
        }
    }
    else {
        //This person suffers from the bug.
        CPDistributedMessagingCenter *center = [CPDistributedMessagingCenter centerNamed:@"com.homeschooldev.fusiond"];
        [center sendMessageName:@"ISufferFromThisToo" userInfo:bug];
        
        button.hidden = YES;
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Bug" message:@"I'm sorry you suffer from this bug too, but it has been noted and the developer is working on it." delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil];
        [alert show];
        [alert release];
    }
}
