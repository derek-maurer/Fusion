...
#import "LIOAuthConsumer.h"

#import "LIRDLinkedInEngine.h"
#import "LIRDLinkedInHTTPURLConnection.h"
#import "LIRDLinkedInRequestBuilder.h"
#import "LIRDLinkedInResponseParser.h"
#import "LIRDLogging.h"
...
@interface LIRDLinkedInEngine ()

- (LIRDLinkedInConnectionID *)sendAPIRequestWithURL:(NSURL *)url HTTPMethod:(NSString *)method body:(NSData *)body;
- (void)sendTokenRequestWithURL:(NSURL *)url token:(LIOAToken *)token onSuccess:(SEL)successSel onFail:(SEL)failSel;

@end


@implementation LIRDLinkedInEngine
...
- (id)initWithConsumerKey:(NSString *)consumerKey consumerSecret:(NSString *)consumerSecret delegate:(id<LIRDLinkedInEngineDelegate>)delegate {
  self = [super init];
  if( self != nil ) {
    rdDelegate = [delegate retain];
    rdOAuthConsumer = [[LIOAConsumer alloc] initWithKey:consumerKey secret:consumerSecret];
...
- (void)closeConnection:(LIRDLinkedInHTTPURLConnection *)connection {
  if( connection ) {
    [connection cancel];
...
- (NSURLRequest *)authorizationFormURLRequest {
  LIOAMutableURLRequest *request = [[[LIOAMutableURLRequest alloc] initWithURL:[NSURL URLWithString:kOAuthAuthorizeURL] consumer:nil token:rdOAuthRequestToken realm:nil signatureProvider:nil] autorelease];
  [request setParameters: [NSArray arrayWithObject: [[[LIOARequestParameter alloc] initWithName:@"oauth_token" value:rdOAuthRequestToken.key] autorelease]]];
  return request;
}
...
- (LIRDLinkedInConnectionID *)updateStatus:(NSString *)newStatus {
  ...
  NSData* body = [LIRDLinkedInRequestBuilder buildSimpleRequestWithRootNode:@"current-status" content:newStatus];
...
- (LIRDLinkedInConnectionID *)sendAPIRequestWithURL:(NSURL *)url HTTPMethod:(NSString *)method body:(NSData *)body {
  ...
  LIOAMutableURLRequest* request = [[[LIOAMutableURLRequest alloc] initWithURL:url
  ...
  LIRDLinkedInHTTPURLConnection* connection = [[[LIRDLinkedInHTTPURLConnection alloc] initWithRequest:request delegate:self] autorelease];
  if( connection ) {
...
- (void)parseConnectionResponse:(LIRDLinkedInHTTPURLConnection *)connection {
  NSError* error = nil;
  id results = nil;
  
  if( [LIRDLinkedInResponseParser parseXML:[connection data] connection:connection results:&results error:&error] ) {
...
- (void)sendTokenRequestWithURL:(NSURL *)url token:(LIOAToken *)token onSuccess:(SEL)successSel onFail:(SEL)failSel {
  LIOAMutableURLRequest* request = [[[LIOAMutableURLRequest alloc] initWithURL:url consumer:rdOAuthConsumer token:token realm:nil signatureProvider:nil] autorelease];
	if( !request ) return;
	
  [request setHTTPMethod:@"POST"];
	if( rdOAuthVerifier.length ) token.pin = rdOAuthVerifier;
	
  LIOADataFetcher* fetcher = [[[LIOADataFetcher alloc] init] autorelease];
  [fetcher fetchDataWithRequest:request delegate:self didFinishSelector:successSel didFailSelector:failSel];
}

- (void)oauthTicketFailed:(LIOAServiceTicket *)ticket data:(NSData *)data {
...
- (void)setRequestTokenFromTicket:(LIOAServiceTicket *)ticket data:(NSData *)data {
  ...
  [rdOAuthRequestToken release];
  rdOAuthRequestToken = [[LIOAToken alloc] initWithHTTPResponseBody:dataString];
...
- (void)setAccessTokenFromTicket:(LIOAServiceTicket *)ticket data:(NSData *)data {
  ...
  [rdOAuthAccessToken release];
  rdOAuthAccessToken = [[LIOAToken alloc] initWithHTTPResponseBody:dataString];
...
- (void)tokenInvalidationSucceeded:(LIOAServiceTicket *)ticket data:(NSData *)data {
  LIOAToken* invalidToken = [rdOAuthAccessToken retain];
  [rdOAuthAccessToken release];
  rdOAuthAccessToken = nil;
...
- (void)connection:(LIRDLinkedInHTTPURLConnection *)connection didReceiveResponse:(NSURLResponse *)response {
  ...
  if( statusCode >= 400 ) {
    ...
    //NSLog(@"delegate: %@",rdDelegate);
    //NSLog(@"Connection: %@",connection);
    //NSLog(@"identifier: %@",connection.identifier);
    [self closeConnection:connection];
  }
}


- (void)connection:(LIRDLinkedInHTTPURLConnection *)connection didReceiveData:(NSData *)data {
  [connection appendData:data];
}


- (void)connection:(LIRDLinkedInHTTPURLConnection *)connection didFailWithError:(NSError *)error {
  if( [rdDelegate respondsToSelector:@selector(linkedInEngine:requestFailed:withError:)] ) {
    [rdDelegate linkedInEngine:self requestFailed:connection.identifier withError:error];
  }
...
- (void)connectionDidFinishLoading:(LIRDLinkedInHTTPURLConnection *)connection {
